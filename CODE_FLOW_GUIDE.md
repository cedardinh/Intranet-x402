...
## 2. 推荐阅读顺序
1. `local/scripts/start-all.mjs`：看环境是如何被一键拉起。
2. `official-x402/examples/typescript/legacy/mcp/index.ts`：看客户端 402 -> 签名 -> 重试逻辑。
3. `official-x402/examples/typescript/servers/express/index.ts`：看资源服务的 paywall 与业务处理边界。
4. `official-x402/e2e/facilitators/typescript/index.ts`：看 verify/settle 与链上执行。
...
2. `local/scripts/README.md`
- 作用：一键脚本行为说明。

### C. 可跳过（当前闭环主线不依赖）
1. `official-x402/typescript/site/**`
2. `official-x402/typescript/packages/**` 中与本地演示无关部分
