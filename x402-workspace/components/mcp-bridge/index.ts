...
function decodeSettlement(headers: HeaderMap): { txHash?: string; network?: string } {
  const encodedPaymentResponse = getHeaderValue(headers, PAYMENT_RESPONSE_HEADER);
  if (!encodedPaymentResponse) return {};
...
const getDataFromResourceServerHandler = async (args: { city?: string }) => {
  ...
  let queryPath = endpointPath;
  if (city) {
    queryPath = appendQueryParam(queryPath, "city", city);
  }
  queryPath = appendQueryParam(queryPath, "traceId", traceId);

  const traceHeaders = { [TRACE_HEADER_NAME]: traceId };
  ...
  if (initialResponse.status !== STATUS_PAYMENT_REQUIRED) {
  ...
  const paymentRequired = httpClient.getPaymentRequiredResponse(
    name => getHeaderValue(initialResponse.headers as HeaderMap, name),
  ...
  if (paidResponse.status !== STATUS_OK) {
  ...
  const settlement = decodeSettlement(paidResponse.headers as HeaderMap);
